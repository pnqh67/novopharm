  ...
  if (check) {
    $.get('/admin/menu/list-menu/' + type, function(json) {
      ...
      $.each(json.data, function(i,e) {
        if(type == 'collection') {
          options += '<option value="/nhom-san-pham/'+e.handle+'">'+e.breadcrumb+'</option>';
          $('.menu-link').find('select[name="menu-collection"]').html(options);
        } else if(type == "blog") {
          options += '<option value="/nhom-bai-viet/'+e.handle+'">'+e.title+'</option>';
          $('.menu-link').find('select[name="menu-blog"]').html(options);
        } else if(type == "article") {
          options += '<option value="/bai-viet/'+e.handle+'">'+e.title+'</option>';
          $('.menu-link').find('select[name="menu-article"]').html(options);
        } else if(type == "page") {
          options += '<option value="/trang-noi-dung/'+e.handle+'">'+e.title+'</option>';
...
$('.btn-create-menu').click(function() {
  ...
  data.title_en = modal.find('input[name="title_en"]').val();
  ...
  else if (data.link_type == 'blog') data.link = modal.find('select[name="menu-blog"]').val();
  else if (data.link_type == 'collection') data.link = modal.find('select[name="menu-collection"]').val();
  else if (data.link_type == 'article') data.link = modal.find('select[name="menu-article"]').val();
  else if (data.link_type == 'page') data.link = modal.find('select[name="menu-page"]').val();
  $.ajax({
    type: 'POST',
    url: '/admin/menu',
    data: data,
...
$('tbody').on('mousedown', '.btn-edit-menu', function(e) {
  ...
  $.get("/admin/menu/"+id, function (json) {
    ...
    if(link_type == 'custom') {
      ...
      modal.find('select[name="menu-blog"]').val(data.link);
      modal.find('select[name="menu-blog"]').removeClass('hidden');
      modal.find('select[name="menu-article"]').addClass('hidden');
      modal.find('select[name="menu-collection"]').addClass('hidden');
      modal.find('input[name="menu-link"]').addClass('hidden');
    }
...
$('.action-box').on('mousedown', '.btn-remove-menu', function(e) {
  ...
  var message = "Khi xóa các menu đã chọn không? Các menu con cũng sẽ bị xóa theo!\nBạn có chắc chắn muốn xóa menu?";
